...
import { httpUpdateOrder } from "@/services/api/requests/order.request";
...
const TradeForm = ({
  ...
  useEffect(() => {
    // Only set default client if no client is provided from state
    if (!clientId && clientsOptions && clientsOptions.length > 0) {
  ...
  }, [clientsOptions, setValue, clientId]);
  ...
  const onSubmit = rest.handleSubmit(async (values) => {
    try {
      // handleSubmit ni chaqirib, natijani kutib olamiz
      await handleSubmit(prepareTradeDto(values), handleReset);
      
      // Agar handleSubmit muvaffaqiyatli bo'lsa, handleStatus ni ishga tushiramiz
      handleStatus();
    } catch (error) {
    ...
    const handleStatus = () => {
      ...
      mutation.mutate({ id: orderId, data });
    }
